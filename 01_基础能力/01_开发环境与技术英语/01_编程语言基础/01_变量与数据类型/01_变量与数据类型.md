...
类型转换在读取文件数据时特别常用——从文件读进来的数据通常是字符串，你需要用 `int()` 或 `float()` 转换成数字才能做数学运算。这一点在 [文件操作与IO](../05_文件操作与IO/) 课程中会有更详细的实践。
...
- 🔗 相关知识点：[字符串与编码](../../09_字符串与编码/) — 深入学习字符串的高级用法
- 🔗 相关知识点：[容器类型深入](../../10_容器类型深入/) — 深入学习列表、字典等容器类型
- 📚 拓展阅读：[Python 数据模型](../../17_元编程与高级特性/05_Python数据模型/) — 了解 Python 对象系统的底层机制
