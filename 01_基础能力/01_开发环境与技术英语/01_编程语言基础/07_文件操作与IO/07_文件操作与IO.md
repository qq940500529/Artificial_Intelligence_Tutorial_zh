...
> ⚠️ **最佳实践**：始终在 `open()` 中显式指定 `encoding="utf-8"`。虽然 Python 3 在大多数系统上默认使用 UTF-8，但在某些 Windows 环境下默认编码可能是 GBK，导致跨平台不兼容。更深入的编码知识会在 [字符串与编码](../../07_字符串与编码/) 课程中讲解。
...
- 🔗 相关知识点：[字符串与编码](../../07_字符串与编码/) — 深入理解 Unicode 和编码问题
