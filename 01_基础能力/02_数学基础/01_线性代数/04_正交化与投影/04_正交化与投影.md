...
[上一节](../03_范数与距离/03_范数与距离.md) 我们学会了用范数测量"向量有多长"、用距离衡量"两个向量有多远"。但你可能注意到，这两个工具其实只回答了"长度"这一类问题。还有一类问题，它们藏在你高中就熟悉的几何里——

> 给你一个点和一条直线，**这个点到直线最近的距离是多少？落脚点在哪？** 

高中时你会从点向直线作垂线，垂足就是答案。这背后有一个隐藏的概念在起作用：**垂直**。垂直为什么这么"灵"？因为垂线段是所有连线中**最短**的——这正是一个"距离最小化"问题的几何答案。

把这个直觉搬到高维向量空间，就是这一节要讨论的两件事：

1. **投影（Projection）**：把一个向量"压"到一条直线、一个平面、一个子空间上，找最近的那个点。这是高中"作垂线"在 $\mathbb{R}^n$ 中的推广。
2. **正交化（Orthogonalization）**：把一组"歪七扭八"的基向量改造成两两垂直的基。这是 [02 线性变换](../02_线性变换/02_线性变换.md) 中"基向量决定一切"思想的进一步深化——既然任何基都能描述空间，为什么不挑一组最好用的？

学完本节，你将拥有理解最小二乘法、PCA、QR 分解、Transformer 注意力机制几何含义的核心工具。
