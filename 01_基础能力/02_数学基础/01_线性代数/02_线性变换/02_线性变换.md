...
在 [上一节](../01_向量与矩阵/01_向量与矩阵.md) 末尾，我们留下了一个关键的视角转换：与其把矩阵 $A$ 看作"一个数表"，不如把它看作"一台机器"——喂给它一个向量 $\mathbf{x}$ ，它吐回另一个向量 $A\mathbf{x}$ 。这个视角让矩阵从静态的数据结构变成了**动态的运算过程**，但当时我们并没有回答一个关键问题：

> 这台机器到底"长什么样"？所有矩阵都对应"同一类"机器吗？还是有些矩阵对应特殊的、有几何意义的变换？

这一节就是来回答这个问题的。我们将看到：**任何矩阵对应的不是任意映射，而是一类性质特别优美的映射——线性变换**。理解了这一点，你不仅能把每一个矩阵在头脑中"画"出来，还会发现：神经网络的每一层（不算激活函数）本质上就是一个线性变换；PCA 找"主方向"、Transformer 中的多头投影、卷积层中的滤波——这些看似不同的操作，几何画面其实是同一套语言。
