...
举个例子，考虑矩阵 $A = \begin{bmatrix} 3 & 0 \\\\ 0 & 2 \end{bmatrix}$ 。它把 $(1, 0)$ 映到 $(3, 0)$ ——方向不变，长度乘了 3 ；把 $(0, 1)$ 映到 $(0, 2)$ ——方向不变，长度乘了 2 。这两个方向就是矩阵 $A$ 的"特殊方向"，对应的拉伸倍数 3 和 2 就是它的"特征值"。
...
**示例**：求 $A = \begin{bmatrix} 4 & 1 \\\\ 2 & 3 \end{bmatrix}$ 的特征分解。
...
| 所有方阵都能对角化 | 错。如 $\begin{bmatrix} 1 & 1 \\\\ 0 & 1 \end{bmatrix}$ 只有一个线性无关的特征向量，不可对角化（叫"亏损矩阵"） |
...
求矩阵 $A = \begin{bmatrix} 2 & 1 \\\\ 1 & 2 \end{bmatrix}$ 的所有特征值与对应特征向量。
...
特征方程 $\det \begin{bmatrix} 2-\lambda & 1 \\\\ 1 & 2-\lambda \end{bmatrix} = (2-\lambda)^2 - 1 = 0$ ，解得 $\lambda_1 = 3, \lambda_2 = 1$ 。
...
$V = \begin{bmatrix} 1 & 1 \\\\ 1 & -1 \end{bmatrix}$ ， $V^{-1} = \dfrac{1}{2} \begin{bmatrix} 1 & 1 \\\\ 1 & -1 \end{bmatrix}$ ， $\Lambda^{10} = \begin{bmatrix} 3^{10} & 0 \\\\ 0 & 1 \end{bmatrix} = \begin{bmatrix} 59049 & 0 \\\\ 0 & 1 \end{bmatrix}$
...
矩阵 $A = \begin{bmatrix} 2 & 0 \\\\ 0 & 3 \\\\ 0 & 0 \end{bmatrix}$ 的 SVD 显然是 $U = I_3, \Sigma = \mathrm{diag}(3, 2)$ 在合适重排下，$V$ 也是简单的置换矩阵。请写出完整的 $U, \Sigma, V$ ，并解释这个 $A$ 的几何含义。
...
$V = \begin{bmatrix} 0 & 1 \\\\ 1 & 0 \end{bmatrix}$ （把 $\mathbf{e}_1, \mathbf{e}_2$ 调换以让大奇异值在前）
