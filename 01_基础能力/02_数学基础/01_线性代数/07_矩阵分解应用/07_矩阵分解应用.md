...
在 [05 特征值与奇异值分解](../05_特征值与奇异值分解/05_特征值与奇异值分解.md) 中，我们已经见识过两种"魔法分解"：把对称矩阵写成 $A = Q\Lambda Q^\top$ ，把任意矩阵写成 $A = U\Sigma V^\top$ 。这种"把复杂矩阵拆成几块结构简单的乘积"的思路，让计算和理解都变得容易了许多。

但你可能也注意到：特征分解和 SVD 计算量都不小。日常工作里我们其实经常碰到一些"轻量级"的需求——

- 给我一个方程组 $A\mathbf{x} = \mathbf{b}$ ，求解；
- 给我一组数据点，做最小二乘拟合；
- 给我一个对称正定矩阵（比如协方差矩阵），快速完成多次反演。

**对于这些更具体的任务，并不需要动用 SVD 这种"重型武器"——线性代数为它们配套了一系列更轻、更快、更稳定的分解方法**：LU、Cholesky、QR、NMF……每一种都是为某一类问题"量身打造"的。这一节把这些经典分解放在一起做横向对比，看看它们各自的形式、原理、适用场景，让你下次面对实际矩阵问题时，能像选工具一样选对方法。
...
$$A = \begin{bmatrix} 1 & 0 \\\\ 1 & 1 \\\\ 1 & 2 \end{bmatrix}, \quad \mathbf{b} = \begin{bmatrix} 1 \\\\ 0 \\\\ 2 \end{bmatrix}$$

利用 QR 分解求最小二乘解 $\mathbf{x}^* = \arg\min \|A\mathbf{x} - \mathbf{b}\|^2$ 。
...
$$R = \begin{bmatrix} \|\mathbf{a}_1\| & \mathbf{a}_2 \cdot \mathbf{q}_1 \\\\ 0 & \|\mathbf{a}_2 - \mathrm{proj}\| \end{bmatrix} = \begin{bmatrix} \sqrt{3} & \sqrt{3} \\\\ 0 & \sqrt{2} \end{bmatrix}$$

第三步：

$$Q^\top \mathbf{b} = \begin{bmatrix} (1+0+2)/\sqrt{3} \\\\ (-1+0+2)/\sqrt{2} \end{bmatrix} = \begin{bmatrix} \sqrt{3} \\\\ 1/\sqrt{2} \end{bmatrix}$$
