...
> 📌 **图解说明**：当我们已知 $B$ 发生后，样本空间被"缩小"为 $B$ 。此时 $A$ 发生的概率就是 $A \cap B$ 在 $B$ 中所占的比例。
...
$$P(C|M) = \dfrac{P(MC)}{P(M)} = \dfrac{0.20}{0.40} = 0.50$$$$P(M|C) = \dfrac{P(MC)}{P(C)} = \dfrac{0.20}{0.30} \approx 0.667$$
