...
在报错信息中，以下语块反复出现。把它们作为整体记忆，你就能快速"扫描"出报错的核心含义：

| 语块 | 中文含义 | 出现场景 |
| ---- | -------- | -------- |
| `is not defined` | 未被定义 | NameError：使用了未声明的变量或函数 |
| `is not callable` | 不可调用 | TypeError：试图调用一个不是函数的对象 |
| `invalid syntax` | 无效语法 | SyntaxError：代码写法不符合规则 |
| `unexpected indent` | 意外的缩进 | IndentationError：缩进格式有问题 |
| `index out of range` | 索引超出范围 | IndexError：访问了列表中不存在的位置 |
| `has no attribute` | 没有该属性 | AttributeError：对象不具备某个方法或属性 |
| `cannot be interpreted as` | 不能被解释为 | TypeError：数据类型不匹配 |
| `expected ... got ...` | 期望…得到… | 参数数量或类型不匹配 |
| `File "...", line N` | 文件"…"，第 N 行 | 指示出错的文件和行号 |
| `in <module>` | 在主模块中 | 错误发生在脚本顶层，不在任何函数内部 |
| `most recent call last` | 最近的调用在最后 | Traceback 的阅读方向提示（从下往上看最直接原因） |
| `did you mean` | 你是不是想… | Python 3.10+ 的拼写建议 |

> 💡 **语块记忆法**：不要孤立地记单词 "defined" 或 "attribute"，而是记住完整的报错语块 "is not defined" 和 "has no attribute"。在真实报错中，这些词总是以固定搭配出现。
...
报错信息虽然看起来各不相同，但它们都遵循固定的结构模式。把这个模式记住，你就能"解码"任何报错：

```
Traceback (most recent call last):      ← 开头标记（表示接下来是调用栈）
  File "文件名", line 行号, in 函数名    ← 出错位置
    出错的那行代码                        ← 出错的代码
错误类型: 错误描述                        ← 核心信息（先看这里！）
```
...
| -------- | -------- |
| 当天 | 浏览"报错常用语块"表格 |
| 第 3 天 | 尝试触发 3 种不同的 Python 错误，练习阅读报错信息 |
| 第 7 天 | 不看答案，完成本节练习题 |
| 第 14 天 | 在实际编程中遇到报错时，先自己分析再搜索 |
