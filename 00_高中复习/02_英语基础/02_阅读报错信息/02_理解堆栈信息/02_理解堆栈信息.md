...
在堆栈追踪中，以下表达反复出现，掌握它们可以极大提升阅读速度：

| 语块 | 中文含义 | 说明 |
| ---- | -------- | ---- |
| `Traceback (most recent call last)` | 回溯（最近调用在最后） | 堆栈的起始标记，提示从底部向上读 |
| `File "...", line N, in func` | 文件"…"，第 N 行，在函数 func 中 | 指示每一层调用的位置 |
| `in <module>` | 在主模块中 | 代码不在任何函数内部，在脚本顶层 |
| `during handling of the above exception` | 在处理上述异常的过程中 | 链式异常：处理异常时又发生了新异常 |
| `The above exception was the direct cause of` | 上述异常是…的直接原因 | 显示异常的因果关系链 |
| `caused by` | 由…引起 | 第三方库中常见的异常链描述 |
| `unsupported operand type(s) for` | 不支持的操作类型 | TypeError 中的典型描述 |

> 💡 **提示**：当你看到 "most recent call last" 时，立刻跳到最后一行看错误类型——这是最高效的阅读策略。
...
训练自己每次看到 Traceback 时都**先看最后一行**。反复练习这个习惯，直到形成条件反射。
...
| -------- | -------- |
| 当天 | 浏览"堆栈信息常用语块"表格 |
| 第 3 天 | 写一段故意嵌套调用的代码，触发 3 层以上的堆栈追踪，练习阅读 |
| 第 7 天 | 不看答案，完成本节练习题 |
| 第 14 天 | 回顾"定位关键词"和本节内容，对比单层和多层报错的阅读技巧 |
